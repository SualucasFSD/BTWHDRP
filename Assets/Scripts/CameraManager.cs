...
public class CameraManager : MonoBehaviour
{
    ...
    [SerializeField][Range(0f, 1f)] private float _shakeIntensity = 0.3f;
    [SerializeField][Range(0f, 2f)] private float _shakeDuration = 0.25f;
    ...
    void Start()
    {
        ...
        EventManager.Suscribe(EventManager.KindOfEvent.MakeCameraShake, Shake);
    ...
    private void OnDestroy()
    {
        ...
        EventManager.Unscribe(EventManager.KindOfEvent.MakeCameraShake, Shake);
    }
}*/
using System;
using System.Linq;
...
public class CameraManager : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        _falseUpdate();
        UpdateShake();
        _cam.transform.position = transform.position + _shakeOffset;
        _cam.transform.rotation = transform.rotation;
    }
    private void UpdateShake()
    {
        if (_shakeTimeRemaining > 0f && ShakeEnabled)
        {
    ...
    private void Shake(params object[] parameters)
    {
        if (parameters.Length > 0)
            ShakeIntensity = (float)parameters[0];
        if (parameters.Length > 1)
            ShakeDuration = (float)parameters[1];

        if (ShakeEnabled)
            _shakeTimeRemaining = ShakeDuration;
    }
    private void SmoothPercent(params object[] p) => _smoothTime = (float)p[0];
    private void DistancePercent(params object[] p) => _distance = (float)p[0];
    public void UpdateMouseSens(params object[] obj) => _sensitivity = (float)obj[0];
    ...
    {
        List<GameObject> Targets = GameManager.Instance.RefreshEnemy(Entity.KindOfEntity.Allies).Select(x => x.gameObject).ToList();
        if (_lookTarget == null || _targets.Count <= 1) return;
    ...
    private void OnEnemyKilledChangeTarget(params object[] obj)
    {
        if ((GameObject)obj[0] != null)
        {
            if (_targets.Contains(obj[0]))
            {
                ChangeLookModeWithOutLineOfSight();
                ChangeLookModeWithOutLineOfSight();
            }
        }
    }

    private void ChangeLookModeWithOutLineOfSight()
    {
        if (_focusing)
        {
            _falseUpdate += RotateCameraLateUpdate;
        ...
        {
            float dist = Vector3.Distance(c.transform.position, _pjModel.transform.position);
            if (dist >= _distanceLocked) continue;
            if (dist < _minDistanceLocked)
            {
                _focusing = true;
                _lookTarget = c.transform;
